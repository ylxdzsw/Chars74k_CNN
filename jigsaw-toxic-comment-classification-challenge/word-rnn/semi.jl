...
function read_data()
    ...
    aX, ay = Array{Any}(length(dict)), Array{Int}(length(dict), 6)

    for i in 1:nrow(test)
        if test[i, 1] in keys(dict)
            j = dict[test[i, 1]]
            aX[j] = X_test[i]
            ay[j, :] = parse.(Int, label[j, 2:end])
        end
    end

    vcat(X_train, aX), X_test, M, vcat(y, ay), ids
end

@main function train(epoch::Int=40)
    X, M, y, ids = if isfile("D:/jigsaw-toxic-comment-classification-challenge/clean/semi-embedded.jld")
        load("D:/jigsaw-toxic-comment-classification-challenge/clean/semi-embedded.jld", "X_train", "M", "y", "ids")
    else
        X_train, X_test, M, y, ids = read_data()
        @save "D:/jigsaw-toxic-comment-classification-challenge/clean/semi-embedded.jld" X_train X_test M y ids
        X_train, M, y, ids
    end
    ...
    for i in 1:epoch
        ...
        i % 10 == 0 && model.save("D:/jigsaw-toxic-comment-classification-challenge/result/semi-word-rnn.model")
    end
end

@main function predict()
    X, M, ids = load("D:/jigsaw-toxic-comment-classification-challenge/clean/semi-embedded.jld", "X_test", "M", "ids")

    model = pywrap(pyimport("model")[:Model](M, "gpu", "D:/jigsaw-toxic-comment-classification-challenge/result/semi-word-rnn.model"))

    open("D:/jigsaw-toxic-comment-classification-challenge/result/semi-word-rnn.submission.csv", "w") do fout
...
@main function feature()
    X_train, X_test, M = load("D:/jigsaw-toxic-comment-classification-challenge/clean/semi-embedded.jld", "X_train", "X_test", "M")

    X = X_train ++ X_test

    model = pywrap(pyimport("model")[:Model](M, "gpu", "D:/jigsaw-toxic-comment-classification-challenge/result/semi-word-rnn.model"))

    open("D:/jigsaw-toxic-comment-classification-challenge/result/semi-word-rnn.feature.csv", "w") do fout
